...
public class IrisObjectDetector {
    ...
    private static final float minimumConfidence = 0.3f;
    ...
    /**
     * reduce image in blue and keep the cropped image for analysis
     */
    public void reduceInBlueAndKeptCropped(Bitmap image, String imagePath) {
...
//     if locations has values (object found), do reduction here
    ...
    /**
     * detection take place once, reduce by blurring
     */
    public void oneShotBlur(Bitmap image, String imagePath) {
        if (this.objectDetector == null) {
            return;
        }
...
//            Log.i(TAG, "reduceImage: result: individual details: " + result.toString() );
            ...
            if (location != null && result.getConfidence() >= minimumConfidence && (title.equals("person") || title.equals("car"))) {
                locations.add(location);
                Log.i(TAG, index + " reduceImage: result: valid details: " + result.toString());
                Log.i(TAG, "detectCarsAndPedestrians: locations300: [" + location.top + ", " + location.left + ", " + location.bottom + ", " + location.bottom + " ]");
            }
        }
        Log.i(TAG, index + " reduceImage: result: valid details: ==================================================================================================");
...
//        maskedImage = ImageUtils.getMaskBitMap(image, locations);
     ...
     * detect cars and pedestrians on the image
     */
    public void loopReductionAndBlue(Bitmap image, String imagePath, int count) {
        if (this.objectDetector == null) {
            return;
        }
        ...
        for (int i = 0; i < count; i++) {
            if (maskedImage == null) {
                Bitmap reducedImage = reduceImage(image, imagePath, i);
                if (reducedImage == null) {
                    break;
                }
                maskedImage = reduceImage(image, imagePath, i);
            } else {
                Bitmap reducedImage = reduceImage(maskedImage, imagePath, i);
                if (reducedImage == null) {
                    break;
                }
                maskedImage = reducedImage;
                ImageUtils.saveBitmap(maskedImage == null? image : maskedImage, imagePath);
            }
